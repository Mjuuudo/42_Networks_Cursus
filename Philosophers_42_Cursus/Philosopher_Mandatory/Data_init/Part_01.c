...
/*   Updated: 2024/11/13 14:31:50 by abait-ou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_forksinit(t_table *table)
{
    ...
    if (pthread_mutex_init(&table->death, NULL) != 0)
        exit (1);  // Todo Replace This Exit WithFucntion That Destroy Previouse Mutexes
...
void ft_philosinit(t_table *table)
{
    ...
    while (counter < table->philos_number)
    {
        ...
        if (counter == 0)
			table->philos[counter].seconde_fork = &table->forks[table->philos_number - 1].fork;
		else
			table->philos[counter].seconde_fork = &table->forks[counter - 1].fork;
