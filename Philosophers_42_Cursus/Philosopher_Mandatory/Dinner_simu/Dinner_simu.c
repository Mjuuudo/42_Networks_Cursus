...
/*   Updated: 2024/11/13 17:49:17 by abait-ou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void threadscreate(t_table *table)
{
    ...
    counter = 0;
    // printf("table data check sss%d %d\n", table->dead, table->end_simu);
    if (pthread_create(&monitor, NULL, &ft_monitor, table) != 0)
    {
            ft_mutexdestroy(table);
            exit (0);
    } // Todo Replace The Exit Line With Mutex Destroy Handling
